...
#ifndef _KONG_TYPES_H_
#define _KONG_TYPES_H_
...
#if defined(_DEBUG)
#if defined(_KONG_WINDOWS_API_) && defined(_MSC_VER) && !defined (_WIN32_WCE)
#if defined(WIN64) || defined(_WIN64) // using portable common solution for x64 configuration
#include <crtdbg.h>
#define _KONG_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_CrtDbgBreak();}
#else
#define _KONG_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_asm int 3}
#endif
#else
#include "assert.h"
#define _KONG_DEBUG_BREAK_IF( _CONDITION_ ) assert( !(_CONDITION_) );
#endif
#else
#define _KONG_DEBUG_BREAK_IF( _CONDITION_ )
#endif

//! Defines a deprecated macro which generates a warning at compile time
/** The usage is simple
For typedef:		typedef _KONG_DEPRECATED_ int test1;
For classes/structs:	class _KONG_DEPRECATED_ test2 { ... };
For methods:		class test3 { _KONG_DEPRECATED_ virtual void foo() {} };
For functions:		template<class T> _KONG_DEPRECATED_ void test4(void) {}
**/
#if defined(IGNORE_DEPRECATED_WARNING)
#define _KONG_DEPRECATED_
#elif _MSC_VER >= 1310 //vs 2003 or higher
#define _KONG_DEPRECATED_ __declspec(deprecated)
#elif (__GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 1)) // all versions above 3.0 should support this feature
#define _KONG_DEPRECATED_  __attribute__ ((deprecated))
#else
#define _KONG_DEPRECATED_
#endif
...
#if defined(_KONG_WINDOWS_API_) && defined(_MSC_VER) && (_MSC_VER > 1299) && (_MSC_VER < 1400)
#define _KONG_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX __asm mov eax,100
#else
#define _KONG_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX
#endif // _KONG_MANAGED_MARSHALLING_BUGFIX


// memory debugging
#if defined(_DEBUG) && defined(KONG_EXPORTS) && defined(_MSC_VER) && \
    (_MSC_VER > 1299) && !defined(_KONG_DONT_DO_MEMORY_DEBUGGING_HERE) && !defined(_WIN32_WCE)
...
#if defined(_KONG_WINDOWS_API_) && defined(_MSC_VER) && (_MSC_VER >= 1400)
...
#endif // __KONG_TYPES_H_INCLUDED__

    ...
    namespace core
    {
        ...
        */
        ...
        public:
            ...
            plane3d() : Normal(0, 1, 0) { recalculateD(Vector<T>(0, 0, 0)); }

            plane3d(const Vector<T>& MPoint, const Vector<T>& Normal) : Normal(Normal) { recalculateD(MPoint); }

            plane3d(T px, T py, T pz, T nx, T ny, T nz) : Normal(nx, ny, nz) { recalculateD(Vector<T>(px, py, pz)); }

            plane3d(const Vector<T>& point1, const Vector<T>& point2, const Vector<T>& point3)
            {
                setPlane(point1, point2, point3);
            }

            plane3d(const Vector<T> & normal, const T d) : Normal(normal), D(d) { }
            ...
            void setPlane(const Vector<T>& point, const Vector<T>& nvector)
            {
                Normal = nvector;
                recalculateD(point);
            }

            void setPlane(const Vector<T>& nvect, T d)
            {
                Normal = nvect;
                D = d;
            }

            void setPlane(const Vector<T>& point1, const Vector<T>& point2, const Vector<T>& point3)
            {
            ...
            */
            bool getIntersectionWithLine(const Vector<T>& linePoint,
                const Vector<T>& lineVect,
                Vector<T>& outIntersection) const
            {
            ...
            */
            f32 getKnownIntersectionWithLine(const Vector<T>& linePoint1,
                const Vector<T>& linePoint2) const
            {
                Vector<T> vect = linePoint2 - linePoint1;
            ...
            */
            bool getIntersectionWithLimitedLine(
                const Vector<T>& linePoint1,
                const Vector<T>& linePoint2,
                Vector<T>& outIntersection) const
            {
            ...
            EIntersectionRelation3D classifyPointRelation(const Vector<T>& point) const
            {
            ...
            void recalculateD(const Vector<T>& MPoint)
            {
                D = -MPoint.DotProduct(Normal);
            }

            //! Gets a member point of the plane.
            Vector<T> getMemberPoint() const
            {
                return Normal * -D;
            }
            ...
            {
                Vector<T> cross = other.Normal.crossProduct(Normal);
            ...
            bool getIntersectionWithPlane(const plane3d<T>& other,
                Vector<T>& outLinePoint,
                Vector<T>& outLineVect) const
            {
            ...
            bool getIntersectionWithPlanes(const plane3d<T>& o1,
                const plane3d<T>& o2, Vector<T>& outPoint) const
            {
                Vector<T> linePoint, lineVect;
            ...
            bool isFrontFacing(const Vector<T>& lookDirection) const
            {
            ...
            /** Note that this only works if the normal is normalized. */
            T getDistanceTo(const Vector<T>& point) const
            {

...
namespace kong
{
    namespace core
    {
        ...
        public:
            ...
            line2d(const Vector<T>& start, const Vector<T>& end) : start(start), end(end) {}
            //! Copy constructor.
            line2d(const line2d<T>& other) : start(other.start), end(other.end) {}

            // operators

            line2d<T> operator+(const Vector<T>& point) const { return line2d<T>(start + point, end + point); }
            line2d<T>& operator+=(const Vector<T>& point) { start += point; end += point; return *this; }
            ...
            //! Set this line to new line going through the two points.
            void setLine(const Vector<T>& nstart, const Vector<T>& nend){ start.set(nstart); end.set(nend); }
            ...
            Vector<T> getVector() const { return Vector<T>(end.X - start.X, end.Y - start.Y); }
            ...
            bool intersectWith(const line2d<T>& l, Vector<T>& out, bool checkOnlySegments = true) const
            {
            ...
            Vector<T> getUnitVector() const
            {
                ...
                return Vector<T>((end.X - start.X) * len, (end.Y - start.Y) * len);
            }
            ...
            T getPointOrientation(const Vector<T>& point) const
            {
            ...
            bool isPointOnLine(const Vector<T>& point) const
            {
            ...
            Vector<T> getClosestPoint(const Vector<T>& point, bool checkOnlySegments = true) const
            {
                Vector<f64> c((f64)(point.X - start.X), (f64)(point.Y - start.Y));
                Vector<f64> v((f64)(end.X - start.X), (f64)(end.Y - start.Y));
                f64 d = v.GetLength();
                ...
                {
                    if (t < 0) return Vector<T>((T)start.X, (T)start.Y);
                    if (t > d) return Vector<T>((T)end.X, (T)end.Y);
                }

                v *= t;
                return Vector<T>((T)(start.X + v.X), (T)(start.Y + v.Y));
            }
        ...
        template <>
        inline Vector2Df line2d<kong::f32>::getClosestPoint(const Vector2Df& point, bool checkOnlySegments) const
        {

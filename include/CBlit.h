...
namespace kong
{
        ...
        const core::position2d<s32>& p1_in)
    {
        ...
        while (code0 | code1)
        {
            ...
            if ((code & CLIPCODE_BOTTOM) == CLIPCODE_BOTTOM)
            {
                ...
                y = clipping.y1;
                x = p0.x_ + (p1.x_ - p0.x_) * (y - p0.y_) / (p1.y_ - p0.y_);
            }
            else
                if ((code & CLIPCODE_TOP) == CLIPCODE_TOP)
                {
                    ...
                    if ((code & CLIPCODE_RIGHT) == CLIPCODE_RIGHT)
                    {
                        // clip right viewport
                        x = clipping.x1;
                        y = p0.y_ + (p1.y_ - p0.y_) * (x - p0.x_) / (p1.x_ - p0.x_);
                    }
                    else
    ...
    */
        ...
        const video::IImage * tex, s32 passnative)
    {
        ...
        {
            out.x0 = clip->UpperLeftCorner.x_;
            out.x1 = clip->LowerRightCorner.x_;
            out.y0 = clip->UpperLeftCorner.y_;
            out.y1 = clip->LowerRightCorner.y_;
            return;
        }
        ...
        if (clip)
        {
            ...
            out.x1 = core::s32_clamp(clip->LowerRightCorner.x_, out.x0, w);
            out.y0 = core::s32_clamp(clip->UpperLeftCorner.y_, 0, h);
            out.y1 = core::s32_clamp(clip->LowerRightCorner.y_, out.y0, h);
        }
        else
        {
            out.x0 = 0;
    ...
    */
        ...
        u32 argb)
    {
        ...
        v.x0 = destPos ? destPos->x_ : 0;
        v.y0 = destPos ? destPos->y_ : 0;
    ...
    static void drawRectangle(video::IImage* img, const core::rect<s32>& rect, const video::SColor &color)
    {
        Blit(color.GetAlpha() == 0xFF ? BLITTER_COLOR : BLITTER_COLOR_ALPHA,
            img, 0, &rect.UpperLeftCorner, 0, &rect, color.color_);
    }
        ...
        const core::position2d<s32>& to, const video::SColor &color)
    {
        ...
        if (ClipLine(clip, p[0], p[1], from, to))
        {
            ...
            case video::ECF_A1R5G5B5:
                if (alpha == 256)
                {
                    RenderLine16_Decal(img, p[0], p[1], video::A8R8G8B8toA1R5G5B5(color.color_));
                }
                else
                {
                    RenderLine16_Blend(img, p[0], p[1], video::A8R8G8B8toA1R5G5B5(color.color_), alpha >> 3);
                }
                break;
            case video::ECF_A8R8G8B8:
                if (alpha == 256)
                {
                    RenderLine32_Decal(img, p[0], p[1], color.color_);
                }
                else
                {
                    RenderLine32_Blend(img, p[0], p[1], color.color_, alpha);
                }
                break;
            default:
                break;

...
#ifdef _DEBUG
#define _KONG_DEBUG_OBJ_LOADER_
#endif
        ...
        IAnimatedMesh* COBJMeshFileLoader::createMesh(io::IReadFile* file)
        {
            ...
            while (bufPtr != bufEnd)
            {
                ...
                case 'v':               // v, vn, vt
                    ...
                    case 't':       // texcoord
                    {
                        ...
                        textureCoordBuffer.PushBack(vec);
                    }
                        break;
                    }
                    break;
                ...
                case 'f':               // face
                {
                    ...
                    if (currMtl)
                        v.color_ = currMtl->Meshbuffer->material_.diffuse_color_;
                    ...
                    while (0 != linePtr[0])
                    {
                        ...
                        v.pos_ = vertexBuffer[Idx[0]];
                        if (-1 != Idx[1])
                            v.texcoord_ = textureCoordBuffer[Idx[1]];
                        else
                            v.texcoord_.Set(0.0f, 0.0f);
                        if (-1 != Idx[2])
                            v.normal_ = normalsBuffer[Idx[2]];
                        else
                        {
                            v.normal_.Set(0.0f, 0.0f, 0.0f);
                        ...
                        {
                            currMtl->Meshbuffer->vertices_.PushBack(v);
                            vertLocation = currMtl->Meshbuffer->vertices_.Size() - 1;
                            currMtl->VertMap.insert(v, vertLocation);
                        }

                        faceCorners.PushBack(vertLocation);
            ...
            SAnimatedMesh* animMesh = nullptr;
        ...
        const c8* COBJMeshFileLoader::readTextures(const c8* bufPtr, const c8* const bufEnd, SObjMtl* currMaterial, const io::path& relPath)
        {
            ...
            while (textureNameBuf[0] == '-')
            {
                ...
                {
                    bufPtr = goAndCopyNextWord(textureNameBuf, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
                    currMaterial->Meshbuffer->material_.material_type_param_ = core::fast_atof(textureNameBuf);
                    bufPtr = goAndCopyNextWord(textureNameBuf, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
                    continue;
                }
                else
            ...
            if (clamp)
                currMaterial->Meshbuffer->material_.SetFlag(video::EMF_TEXTURE_WRAP, video::ETC_CLAMP);
            ...
            if (texname.size())
            {
                // TODO: user's dir should be searched
                ...
                {
                    newTexture = SceneManager->GetVideoDriver()->FindTexture(texname) == nullptr;
                    texture = SceneManager->GetVideoDriver()->GetTexture(texname);
                }
                else
                {
                    newTexture = SceneManager->GetVideoDriver()->FindTexture(relPath + texname) == nullptr;
            ...
            {
                if (type == 0)
                    currMaterial->Meshbuffer->material_.SetTexture(0, texture);
                else if (type == 1)
                {
                    if (newTexture)
                        SceneManager->GetVideoDriver()->MakeNormalMapTexture(texture, bumpiness);
                    currMaterial->Meshbuffer->material_.SetTexture(1, texture);
                    currMaterial->Meshbuffer->material_.MaterialType = video::EMT_PARALLAX_MAP_SOLID;
                    currMaterial->Meshbuffer->material_.material_type_param_ = 0.035f;
                }
                else if (type == 2)
                {
                    currMaterial->Meshbuffer->material_.SetTexture(0, texture);
                    currMaterial->Meshbuffer->material_.MaterialType = video::EMT_TRANSPARENT_ADD_COLOR;
                }
                else if (type == 3)
                {
                ...
                currMaterial->Meshbuffer->material_.diffuse_color_.Set(
                    currMaterial->Meshbuffer->material_.diffuse_color_.GetAlpha(), 255, 255, 255);
            }
            return bufPtr;
        }
        ...
        void COBJMeshFileLoader::readMTL(const c8* fileName, const io::path& relPath)
        {
            ...
            while (bufPtr != bufEnd)
            {
                ...
                case 'n': // newmtl
                {
                    ...
                    if (currMaterial)
                        Materials.PushBack(currMaterial);
                ...
                case 'N':
                    if (currMaterial)
                    {
                        ...
                        case 's': // Ns - shininess
                        {
                            ...
                            currMaterial->Meshbuffer->material_.shininess_ = shininessValue;
                        }
                            break;
                ...
                case 'K':
                    if (currMaterial)
                    {
                        ...
                        case 'd':		// Kd = diffuse
                        {
                            bufPtr = readColor(bufPtr, currMaterial->Meshbuffer->material_.diffuse_color_, bufEnd);
                        ...
                        case 's':		// Ks = specular
                        {
                            bufPtr = readColor(bufPtr, currMaterial->Meshbuffer->material_.specular_color_, bufEnd);
                        }
                            break;

                        case 'a':		// Ka = ambience
                        {
                            bufPtr = readColor(bufPtr, currMaterial->Meshbuffer->material_.ambient_color_, bufEnd);
                        }
                            break;
                        case 'e':		// Ke = emissive
                        {
                            bufPtr = readColor(bufPtr, currMaterial->Meshbuffer->material_.emissive_color_, bufEnd);
                        }
                            break;
                        }	// end switch(bufPtr[1])
                ...
                case 'd': // d - transparency
                    if (currMaterial)
                    {
                        ...
                        currMaterial->Meshbuffer->material_.diffuse_color_.SetAlpha(static_cast<s32>(dValue * 255));
                        if (dValue<1.0f)
                            currMaterial->Meshbuffer->material_.MaterialType = video::EMT_TRANSPARENT_VERTEX_ALPHA;
                    }
                    break;
                case 'T':
                    if (currMaterial)
                    {
                        ...
                        case 'f':		// Tf - Transmitivity
                            ...
                            currMaterial->Meshbuffer->material_.diffuse_color_.SetAlpha((s32)(transparency * 255));
        ...
        const c8* COBJMeshFileLoader::readVec3(const c8* bufPtr, core::vector3df& vec, const c8* const bufEnd)
        {
            ...
            vec.x_ = -core::fast_atof(wordBuffer); // change handedness
            bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
            vec.y_ = core::fast_atof(wordBuffer);
            bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
            vec.z_ = core::fast_atof(wordBuffer);
            return bufPtr;
        }
        ...
        const c8* COBJMeshFileLoader::readUV(const c8* bufPtr, core::vector2df& vec, const c8* const bufEnd)
        {
            ...
            bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
            vec.x_ = core::fast_atof(wordBuffer);
            bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
            vec.y_ = 1 - core::fast_atof(wordBuffer); // change handedness
            return bufPtr;
        }
        ...
        COBJMeshFileLoader::SObjMtl* COBJMeshFileLoader::findMtl(const core::stringc& mtlName, const core::stringc& grpName)
        {
            ...
            if (defMaterial)
            {
                Materials.PushBack(new SObjMtl(*defMaterial));
            ...
            {
                Materials.PushBack(new SObjMtl(*Materials[0]));
                Materials.GetLast()->Group = grpName;
                return Materials.GetLast();
            }
            return 0;

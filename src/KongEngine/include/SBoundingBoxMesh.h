...
namespace kong
{
    namespace scene
    {
        ...
        inline void SBoundingBoxMesh::RebuildBoundingBoxMesh(const core::aabbox3d<f32> box) const
        {
            ...
            core::Array<video::S3DVertex> *vertices = static_cast<core::Array<video::S3DVertex> *>(mesh_->GetMeshBuffer(0)->GetVertices());
            ...
            (*vertices)[5].pos_ = core::vector3df(right, top, back);
            (*vertices)[6].pos_ = core::vector3df(right, bottom, back);
            ...
            (*vertices)[22].pos_ = core::vector3df(left, bottom, front);
        }
